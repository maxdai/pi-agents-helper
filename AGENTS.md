...
5. **测试输出缓存（tests/run_tests.sh，用户 2026-09-01 想法）**：全量测试
   250s+，同一输出多次观察（Ran/OK/详情）重跑浪费——源码（*.py/*.sh/*.tpl
   内容 md5）指纹 + 命令参数为缓存 key；命中且上次 OK → 直接输出缓存（0 秒）；
   **上次 FAILED 的缓存不可信**（flaky/环境变化）→ 重跑确认；`--force`
   强制重跑。环境因素（残留进程/负载）不体现在指纹里——命中可信度等同
   当时，想重验用 --force。
